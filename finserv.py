...
class EnhancedCVMForce:
    ...
    def _smart_modify(self, data: bytes, tracker, key: Optional[str] = None) -> bytes:
        ...
        rebuilt, modified = self._rebuild_children(parsed, tracker, key)
        return rebuilt if modified else data

    def _rebuild_children(self, children, tracker, key: Optional[str] = None):
        """Recursively rebuild parsed TLVs and report whether anything changed."""
        new_data = bytearray()
        modified = False

        for tag, value, nested_children in children:
            new_value = value
            ...
            if nested_children:
                new_value, child_modified = self._rebuild_children(
                    nested_children, tracker, key
                )
                modified = modified or child_modified

            new_data.extend(self.parser.build_tlv(tag, new_value))

        return bytes(new_data), modified
    ...
    def apply_to_nfcdata(self, nfc_data: NFCData, client_id: str, tracker, key=None) -> NFCData:
        try:
            if nfc_data.data and len(nfc_data.data) >= 2:
